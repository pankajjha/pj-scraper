...
fakeSearchEngine.get('/search', (req, res) => {
    ...
    res.sendFile(path.join(__dirname, '../mocks/google-mobile/' + req.query.q + '_page' + pageNumber + '.html'));
});
fakeSearchEngine.use(express.static('test/mocks/google-mobile', {extensions: ['html']}));
...
describe('Module Google Mobile', function(){
    ...
    it('one keyword one page', function(){
        const googleMobileScraper = new GoogleMobileScraper({
            config: {
                search_engine_name: 'google_mobie',
